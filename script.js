...
export function aggiungi(espressione, carattere) {
	let input = espressione;
	if (input.charAt(0) == "0" && !isOperand(carattere)) {
...
// funzione per cancellare un carattere
export function  cancella(espressione) {
    let input = espressione;
    let remove = 1;
    if (input.length >= 3) {
		let n = input.slice(input.length - 3, input.length);
		if (isFunction(n) || n == "NaN" || n == "Ans") // n = ultime 3,  NaN= Not a Number
			remove = 3;
	}
	input = espressione.slice(0, input.length - remove);
	return input;
}
// cancellare tutta l'espressione
